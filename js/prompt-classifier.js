...
export function classifyPrompt(text = "") {
  ...
  if (!prompt) return { profileId: "", confidence: 0, evidence: [], taskNumber: null, examModule: "" };

  const letterHits = LETTER_PATTERNS.filter((pattern) => pattern.test(prompt)).map((pattern) => pattern.source);
  const visualHits = VISUAL_PATTERNS.filter((pattern) => pattern.test(prompt)).map((pattern) => pattern.source);
  const task2Hits = TASK2_PATTERNS.filter((pattern) => pattern.test(prompt)).map((pattern) => pattern.source);

  if (letterHits.length >= 2 || (letterHits.length && /bullet|•|\n\s*[-–—]/.test(prompt))) {
    return {
  ...
  if (visualHits.length >= 2 || (visualHits.length && /below|shows?|illustrates?|compares?/i.test(prompt))) {
    return {
      profileId: "academic_task1",
      examModule: "academic",
      taskNumber: 1,
      confidence: Math.min(0.99, 0.7 + visualHits.length * 0.1),
      evidence: ["题目包含图表/地图/流程图特征", visualHits.length >= 2 ? "检测到标准 Academic Task 1 指令" : "检测到视觉描述任务"]
    };
  }
  ...
  return { profileId: "", examModule: "", taskNumber: null, confidence: 0.35, evidence: ["题目特征不足，继续使用用户选择"] };
}

export function detectConflict(selectedProfileId, detection) {
  if (!detection?.profileId || detection.profileId === "task2_ambiguous") {
    if (detection?.taskNumber === 2 && !String(selectedProfileId).endsWith("task2")) {
      return { conflict: true, suggestedProfileId: "", message: "系统判断这更像 Task 2 题目，请确认你选择的任务。" };
    }
    return { conflict: false, suggestedProfileId: "", message: "" };
  }
  if (detection.profileId === selectedProfileId) return { conflict: false, suggestedProfileId: "", message: "" };
  return {
    conflict: true,
    suggestedProfileId: detection.profileId,
    message: detection.profileId === "academic_task1"
